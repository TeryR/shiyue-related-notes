...
- **相关原因标签**：每条推荐下面显示两篇笔记的共同主题词（本地规则提取）；开启「AI 智能标签」后用对话模型生成更准的词组，生成失败自动回落规则标签，不阻塞推荐
- **语义搜索**：用自然语言搜，输入即自动检索，不用记得关键词；结果带命中片段摘要，直接定位到原文位置
- **混合检索**：向量 + 关键词（BM25 + RRF）两路一起查，人名、代码、型号这类精确词也能搜到
- **重排序精排（可选，进阶）**：配有 rerank 端点（Ollama 或 OpenAI 兼容，如硅基流动 `BAAI/bge-reranker-v2-m3`）后，混合检索粗排的结果再精排一遍，头部更准；接口失败自动回落混合检索排序
- **长笔记分块索引**：长文切成语义块（结构感知 + 重叠）逐块索引，修改只重嵌变化的块，不整篇重来
- **Smart View**：在笔记里写 ` ```smart-connections ` 代码块内嵌相似笔记列表，语法与原版兼容
- **智能对话**：基于你自己的笔记回答问题（自动检索相关笔记当上下文，显示"参考了 N 篇笔记"）；流式打字机输出，可随时停止，一键保存对话为笔记
- **改写选中文本 / 从选中内容生成笔记**：流式输出，改完可直接替换原文或复制，生成的笔记自动起标题保存
- **增量索引 + 后台队列**：笔记新增、修改、删除后台自动更新；可随时停止（已完成部分保留）、继续索引（增量补齐，只补变化的）、重建、清空；状态栏实时显示进度，索引出错会变红、点击查看原因
- **换模型不清空索引**：切换嵌入模型后旧索引继续可用；想用新模型重算时点「继续索引」会先提示确认
- **排除文件夹**：设了排除的笔记不进索引（变更后会自动从索引移除）
...
| 对话模型（可选） | 只用推荐/搜索可以完全不配。要用智能对话/改写/生成/AI 智能标签，再配一个：Ollama `qwen2.5:7b`（免费离线）或 OpenAI 兼容接口或 Anthropic 接口 |
| 重排序端点（可选） | 进阶功能，不配则自动用混合检索排序 |
